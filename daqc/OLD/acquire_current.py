...
def LoopAcquire():
    ...
    while (global_error_code >= 0):
        ...
        for channel_index in range(0, 16):
            cond_sec = downsample(scale_cond_transmitter(current[SAMPLES_PER_CHAN*(channel_index+0):SAMPLES_PER_CHAN*(channel_index+1)]), 1)
            print(cond_sec)
            try:
                filename_cond = repr(97+channel_index) + "_" + repr(prev_acq_finish_secs)
                numpy.save(FILE_PATH+filename_cond, cond_sec)
