...
class Device:
    ...
    def __init__(self, sample_rate = 1, samplesPerChan = 1, subSamplesPerChan = 1, minValue = 0, maxValue = 10, IPNumber = "", moduleSlotNumber = 1, moduleChans = [0], uniqueChans = [0]):
        ...
        self.moduleChans = numpy.array(moduleChans)
        self.uniqueChans = numpy.array(uniqueChans)
...
class Nidaq(Device):
    ...
    def __init__(self, sample_rate = 1, samplesPerChan = 1, subSamplesPerChan = 1, minValue = 0, maxValue = 10, IPNumber = "", moduleSlotNumber = 1, moduleChans = [0], uniqueChans = [0]):
        ...
        Device.__init__(self, sample_rate, samplesPerChan, subSamplesPerChan, minValue, maxValue, IPNumber, moduleSlotNumber, moduleChans, uniqueChans)
...
class NidaqVoltageIn(Nidaq):
    ...
    def SetupModule(self):
        ...
        print( "moduleChanNamesArray[moduleChans]: ", repr(self.moduleChanNamesArray[self.moduleChans]) )
        moduleChanNamesSelected = ctypes.create_string_buffer(str.encode((",".join((self.moduleChanNamesArray[self.moduleChans]).tolist()))))
    ...
    def LoopAcquire(self):
        ...
        while (self.globalErrorCode >= 0):
            ...
            noOfChans = len(self.moduleChans)
            for chanIndex in range(0, noOfChans):
                filename = repr(self.uniqueChans[chanIndex]) + "_" + repr(prevAcqFinishSecs)
